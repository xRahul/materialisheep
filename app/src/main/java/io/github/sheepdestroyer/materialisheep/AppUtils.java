 ...
 * handling URLs, text formatting, network checks, and UI manipulations.
 */
public class AppUtils {
  ...
  public static void setTextWithLinks(TextView textView, CharSequence html) {
    ...
    textView.setOnTouchListener(
        new LinkTouchListener() {
          @Override
          public void onLinkClick(TextView widget, ClickableSpan span) {
            if (span instanceof URLSpan) {
              openWebUrlExternal(
                  widget.getContext(), null, ((URLSpan) span).getURL(), null);
            } else {
              span.onClick(widget);
            }
          }
  ...
  public static void openExternal(
    ...
    if (TextUtils.isEmpty(item.getUrl())
      ...
      openWebUrlExternal(
          context, item, String.format(HackerNewsClient.WEB_ITEM_PATH, item.getId()), session);
      return;
    }
    popupMenu
        .create(context, anchor, GravityCompat.END)
  ...
  public static int getDimensionInDp(Context context, @DimenRes int dimenResId) {
    return (int)
        (context.getResources().getDimension(dimenResId)
  ...
  /**
   * Displays UI to allow user to login If no accounts exist in user's device, regardless of login
   * status, prompt to login again If 1 or more accounts in user's device, and already logged in,
   * prompt to update password If 1 or more accounts in user's device, and logged out, show account
   * chooser
   *
  ...
  public static void showLogin(Context context, AlertDialogBuilder alertDialogBuilder) {
    ...
    if (accounts.length == 0) { // no accounts, ask to login or re-login
      context.startActivity(new Intent(context, LoginActivity.class));
        ...
        Preferences.getUsername(context))) { // stale account, ask to re-login
      context.startActivity(new Intent(context, LoginActivity.class));
  ...
  public static void showAccountChooser(
      final Context context, AlertDialogBuilder alertDialogBuilder, Account[] accounts) {
    String username = Preferences.getUsername(context);
   ...
   */
  public static void toggleFabAction(
      FloatingActionButton fab, WebItem item, boolean commentMode, View.OnClickListener listener) {
    Context context = fab.getContext();
    fab.setImageResource(
        commentMode ? R.drawable.ic_reply_white_24dp : R.drawable.ic_zoom_out_map_white_24dp);
  ...
  public static String toHtmlColor(Context context, @AttrRes int colorAttr) {
    return String.format(
        FORMAT_HTML_COLOR,
        0xFFFFFF & ContextCompat.getColor(context, AppUtils.getThemedResId(context, colorAttr)));
  }
        ...
        dim
            ...
            : ContextCompat.getColor(
                ...
                AppUtils.getThemedResId(
                    window.getContext(), androidx.appcompat.R.attr.colorPrimaryDark)));
  }
  ...
  public static LayoutInflater createLayoutInflater(Context context) {
    return LayoutInflater.from(
        new ContextThemeWrapper(context, Preferences.Theme.resolvePreferredTextSize(context)));
  }
        ...
        new Intent(Intent.ACTION_SEND)
            ...
            .putExtra(
                ...
                !TextUtils.isEmpty(subject)
                    ? TextUtils.join(" - ", new String[] {subject, text})
                    : text);
    if (intent.resolveActivity(context.getPackageManager()) != null) {
      context.startActivity(intent);
    }
